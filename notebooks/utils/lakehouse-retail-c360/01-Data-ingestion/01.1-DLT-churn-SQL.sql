...
-- MAGIC %md
-- MAGIC Your DLT Pipeline has been installed and started for you! Open the <a dbdemos-pipeline-id="dlt-churn" href="#joblist/pipelines/a959a596-f1dd-4529-b0e8-d68458ec1a62" target="_blank">Churn Delta Live Table pipeline</a> to see it in action.<br/>
...
-- MAGIC %python
-- MAGIC display(spark.read.json('/Volumes/main/dbdemos_retail_c360_DIDayMumbai/c360/users'))

-- COMMAND ----------

-- DBTITLE 1,Raw incoming orders (json)
-- MAGIC %python
-- MAGIC display(spark.read.json('/Volumes/main/dbdemos_retail_c360_DIDayMumbai/c360/orders'))

-- COMMAND ----------

-- DBTITLE 1,Raw incoming clickstream (csv)
-- MAGIC %python
-- MAGIC display(spark.read.csv('/Volumes/main/dbdemos_retail_c360_DIDayMumbai/c360/events', header=True))
...
AS SELECT * FROM cloud_files("/Volumes/main/dbdemos_retail_c360_DIDayMumbai/c360/events", "csv", map("cloudFiles.inferColumnTypes", "true"))
...
AS SELECT * FROM cloud_files("/Volumes/main/dbdemos_retail_c360_DIDayMumbai/c360/orders", "json")
...
AS SELECT * FROM cloud_files("/Volumes/main/dbdemos_retail_c360_DIDayMumbai/c360/users", "json", map("cloudFiles.inferColumnTypes", "true"))
...
-- MAGIC #                                                                 Model name         |        
-- MAGIC #                                                                     |              |        
-- MAGIC predict_churn_udf = mlflow.pyfunc.spark_udf(spark, "models:/main.dbdemos_retail_c360_DIDayMumbai.dbdemos_customer_churn@prod", "int")
...
-- MAGIC
-- MAGIC Open the <a dbdemos-pipeline-id="dlt-churn" href="#joblist/pipelines/a959a596-f1dd-4529-b0e8-d68458ec1a62" target="_blank">Churn Delta Live Table pipeline</a> and click on start to visualize your lineage and consume the new data incrementally!
...
-- MAGIC
-- MAGIC <a href="/sql/dashboards/f44baa30-5d19-472d-b1b5-cb101bbb652c-dlt---retail-data-quality-stats" target="_blank">Data Quality Dashboard</a>
...
-- MAGIC
-- MAGIC <a href="/sql/dashboards/a3ca488a-ffcf-45dc-b302-e5ff92b0ec14" target="_blank">Open the DBSQL Dashboard</a>
